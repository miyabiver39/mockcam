...
- REST と MCP の両方に影響する変更（プロファイル操作・PTZ・スナップショット等）は `internal/camera/controller.go` で行い、`camera` / `web` / `mcpserver` の 3 つのテストを回す。
- API を追加・変更したら `docs/openapi.yaml` を更新する（`internal/web/api_docs_test.go` が spec とルータの整合と `info.version` == `AppVersion` を検査する）。
...
- `README.md` は英語、`README.jp.md` は日本語で両方を同期して更新する。`docs/*.md` は日本語、`docs/openapi.yaml` は英語。コード内コメントと doc コメントは英語。
